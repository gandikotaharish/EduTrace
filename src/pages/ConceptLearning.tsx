...
import { SecureTextarea } from "@/components/integrity/SecureTextarea";
import { FocusGuard } from "@/components/integrity/FocusGuard";
import { MinTimeGate, TimeRemainingIndicator } from "@/components/integrity/MinTimeGate";
...
// Minimum time per step in seconds
const MIN_STEP_TIMES: Record<string, number> = {
  thinking: 45,
  reflection: 30,
  application: 45,
};
...
export default function ConceptLearning() {
  ...
  const handleViolation = useCallback((type: 'paste_attempt' | 'suspicious_typing') => {
    logViolation(type, conceptId);
  }, [logViolation, conceptId]);

  const handleTabSwitch = useCallback(() => {
    logViolation('tab_switch', conceptId);
  }, [logViolation, conceptId]);
  ...
  const handleNext = async () => {
    ...
    // Check for shallow reflections
    if (currentStep === 'reflection') {
      const genericPhrases = ['i understood everything', 'nothing was confusing', 'it was easy', 'no mistakes'];
      const isShallow = confusionPoint.length < 20 || 
        genericPhrases.some(p => confusionPoint.toLowerCase().includes(p));
      if (isShallow && confusionPoint.length >= 10) {
        logViolation('shallow_reflection', conceptId, confusionPoint);
      }
    }
  ...
  const minTime = MIN_STEP_TIMES[currentStep] || 0;

  return (
    <DashboardLayout>
      <FocusGuard active={isActiveStep} onTabSwitch={handleTabSwitch}>
        <div className="max-w-4xl mx-auto">
            ...
            <div className="flex items-center justify-between mb-4">
              ...
              {integrityData && (
                <div className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-muted/50 border">
                  ...
                  <IntegrityBadge score={integrityData.score} size="sm" showLabel={false} />
                </div>
              )}
            </div>
              ...
              {steps.map((step, i) => {
                ...
                return (
                  <div key={step.key} className="flex items-center">
                    ...
                    {i < steps.length - 1 && (
                      <div className={cn("w-8 h-0.5 mx-2", isComplete ? "bg-success" : "bg-border")} />
                    )}
                  </div>
                );
              })}
          ...
          <div className="evidence-card mb-6">
            ...
            {currentStep === 'thinking' && (
              <div className="space-y-6">
                ...
                <p className="text-muted-foreground">{content.thinking_task_prompt}</p>
              ...
              <div className="space-y-6">
                ...
                <div className="space-y-4">
                  <div className="space-y-2">
                    ...
                    <SecureTextarea
                      placeholder="Be specific about what confused you — generic answers are flagged..."
                  ...
                  <div className="space-y-2">
                    ...
                    <SecureTextarea
                      ...
                      onChange={setMistakeDescription}
                      onViolation={handleViolation}
            ...
            {currentStep === 'application' && (
              <div className="space-y-6">
                ...
                <p className="text-muted-foreground">{content.micro_app_prompt}</p>
              ...
              <div className="text-center py-12">
                ...
                {integrityData && (
                  <div className="flex justify-center mb-6">
                    <div className="bg-muted/50 rounded-lg px-6 py-3 flex items-center gap-3">
                      <ShieldCheck className="w-5 h-5 text-muted-foreground" />
                      <span className="text-sm text-muted-foreground">Integrity Score:</span>
                      <IntegrityBadge score={integrityData.score} size="sm" />
                    </div>
                  </div>
                )}
                <div className="flex gap-4 justify-center">
                  <Button variant="outline" onClick={() => navigate('/student')}>Back to Dashboard</Button>
                  <Button variant="hero" onClick={() => navigate('/student/learn')}>
          ...
          {/* Navigation with MinTime gate */}
          {currentStep !== 'complete' && (
            <div className="space-y-4">
              {minTime > 0 && (
                <MinTimeGate minSeconds={minTime} startTime={stepStartTime}>
                  {(_canProceed, remaining) => (
                    <TimeRemainingIndicator seconds={remaining} />
                  )}
                </MinTimeGate>
              )}
                ...
                <MinTimeGate minSeconds={minTime} startTime={stepStartTime}>
                  {(timeOk) => (
                    <Button
                      variant="hero"
                      onClick={handleNext}
                      disabled={!canProceed() || !timeOk || submitEvidence.isPending}
                    >
